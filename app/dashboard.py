...
def extract_features_for_knn(audio_path, feature_names, sr=22050, segment_seconds=3.0, max_segments=10):
    y, sr = librosa.load(audio_path, sr=sr, mono=True)
    segment_length = int(segment_seconds * sr)

    rows = []
    total_segments = min(max_segments, max(1, len(y) // segment_length))
    ...
    if uploaded_file is None:
        ...
        if st.button("Dự đoán bằng KNN", key="knn_predict"):
            try:
                ...
                X_features, y_audio, sr = extract_features_for_knn(audio_path, feature_names)
                X_scaled = scaler.transform(X_features)

                segment_probs = knn.predict_proba(X_scaled)
                ...
                c3.metric("Số đoạn phân tích", len(X_features))
